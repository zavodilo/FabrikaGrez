...
try {
    ...
    await st(() => {
        for (let i = 0; i < 120 && MovieSequencer.playing; i++) {
            MovieSequencer.update(0.25);
            if (MovieSequencer.si >= 1 && MovieSequencer.state === 'scene') break;
        }
        for (let i = 0; i < 12 && MovieSequencer.playing; i++) MovieSequencer.update(0.25);
    });
    await sleep(400);
