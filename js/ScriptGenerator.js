...
const ScriptGenerator = {
    ...
    _scene(sc, script, idOf, extraIds, slots, r, isLast) {
        ...
        const enter = [];
        for (const k of keys) {
            enter.push({ who: idOf[k], anchor: at[idOf[k]], heading: null, act: 'idle' });
        }
        // A couple of background faces in the wide shots.
        const crowdN = Math.min(extraIds.length, sc.kind === 'intro' || sc.kind === 'climax' || sc.kind === 'coda' ? 3 : 1);
        for (let i = 0; i < crowdN; i++) {
            enter.push({ who: extraIds[i], anchor: S[order[(oi + i) % order.length]] || S.wide, act: i % 2 ? 'idle' : 'talk' });
        }
        ...
        for (let i = 0; i < (sc.props || []).length; i++) {
            const id = sc.props[i];
            const anchor = { anchor: S[order[(oi + i + 1) % order.length]] || S.c, dx: 46 + i * 22, dy: -34 };
        ...
        for (let si = 0; si < recipe.length; si++) {
            ...
            const cam = this._cam(camType, subj, A, B, S, at);
            if (!cam) continue;
            const beats = [];
            const first = si === 0;
            ...
            shots.push({ dur: dur, trans: first && si === 0 ? 'fade' : (camType === 'wide' && si === 0 ? 'fade' : 'cut'), cam: cam, beats: beats });
        }
    ...
    _cam(type, subj, A, B, S, at) {
        if (type === 'wide' || type === 'crane') {
            const anchor = subj === 'c' ? (S.wide || S.c) : (S.wide || S.c);
        ...
        if (type === 'duo') {
            if (!B) return { type: 'medium', who: A, pitch: 12, zoom: 2.1 };
            return { type: 'duo', who: A, who2: B, pitch: 11, zoom: 1.9 };
        }
        if (type === 'over') {
            if (!B) return { type: 'medium', who: A, pitch: 10, zoom: 2.2 };
            // 'ab' — over B's shoulder at A; 'ba' — the reverse.
            return subj === 'ba' ? { type: 'over', who: B, who2: A, zoom: 2.8 } : { type: 'over', who: A, who2: B, zoom: 2.8 };
        }
        const who = subj === 'b' && B ? B : A;
        if (type === 'close') return { type: 'close', who: who, pitch: 7, zoom: 4.1 };
        if (type === 'dutch') return { type: 'dutch', who: who, pitch: 5, zoom: 4.3, roll: 9 };
        if (type === 'low') return { type: 'low', who: who, pitch: -5, zoom: 2.6 };
        return { type: 'medium', who: who, pitch: 12, zoom: 2.1 };
    },
