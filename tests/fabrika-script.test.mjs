...
test('MovieData.ANCHOR_XY совпадает с якорями движка (иначе постановка слепая)', () => {
    const xy = MovieData.ANCHOR_XY;
    assert.ok(xy && Object.keys(xy).length, 'ANCHOR_XY пуст');
    for (const set of Object.keys(MovieData.SET_INFO)) {
        assert.ok(ANCHORS[set] && ANCHORS[set].size, set + ': у декорации нет якорей в движке');
        assert.ok(xy[set], 'нет координат для декорации ' + set);
        for (const name of ANCHORS[set]) {
            assert.ok(Array.isArray(xy[set][name]), set + '.' + name + ': нет координат');
            assert.equal(xy[set][name].length, 2);
            for (const v of xy[set][name]) assert.ok(Number.isFinite(v), set + '.' + name + ' не число');
        }
        // Every slot point the generator stages at must have coordinates.
        for (const sl of SLOTS) {
            const nm = MovieData.SET_SLOTS[set][sl];
            assert.ok(xy[set][nm], set + ': слот ' + sl + ' (' + nm + ') без координат');
        }
    }
});

test('постановка: говорящие развёрнуты друг к другу, камера близкого плана на оси диалога', () => {
    const s = stateWith(8);
    let checked = 0;
    for (const genre of GENRES) {
        for (const budget of [300000, 1500000]) {
            const sc = ScriptGenerator.draft(s, { genre, budget, seed: Rng.hash('face-' + genre + budget) });
            const castByKey = {};
            const r = Rng.create('face-cast-' + genre);
            for (const role of sc.roles) castByKey[role.key] = PeopleSystem.randomPerson(r, { role: 'actor', minSkill: 5, maxSkill: 9 });
            const tl = ScriptGenerator.compile(sc, castByKey, s);
            for (const scn of tl.scenes) {
                const slots = MovieData.SET_SLOTS[scn.set];
                const xy = MovieData.ANCHOR_XY[scn.set];
                const enterByWho = {};
                for (const e of scn.enter) enterByWho[e.who] = e;
                // Two speakers must face each other (headings opposite within tolerance).
                const a = enterByWho.a1, b = enterByWho.a2;
                if (a && b && a.heading != null && b.heading != null) {
                    const pa = xy[slots[Object.keys(slots).find((k) => slots[k] === a.anchor)] ? 'a' : 'a'];
                    // The heading of a must point at b's anchor.
                    const nameA = a.anchor, nameB = b.anchor;
                    const va = xy[nameA], vb = xy[nameB];
                    if (va && vb) {
                        const want = Math.atan2(vb[1] - va[1], vb[0] - va[0]) * 180 / Math.PI;
                        const d = Math.abs(((a.heading - want + 540) % 360) - 180);
                        assert.ok(d < 1.5, scn.label + ': актёр a1 смотрит не на партнёра (' + d.toFixed(1) + '°)');
                        checked++;
                    }
                }
                // Every actor-framed shot carries a pinned azimuth.
                for (const sh of scn.shots) {
                    if (['close', 'medium', 'dutch', 'low'].includes(sh.cam.type)) {
                        assert.ok(sh.cam.az != null && Number.isFinite(sh.cam.az), scn.label + ': кадр ' + sh.cam.type + ' без az');
                    }
                }
            }
        }
    }
    assert.ok(checked >= GENRES.length, 'ни разу не проверено встречное направление взгляда');
});
