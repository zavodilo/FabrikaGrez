// Gltf3D per-view model cache: a stale container (asset unloaded after the last instance of
// a shared container died) must NOT be handed out again — the western→forest render crash
// ('reading impl': destroyed meshes in the render queue) came from exactly that. Regression:
// load -> unload behind the cache -> load again must produce a FRESH asset and container.
...
test('cache hands the same model out while the container asset is alive', async () => {
    ...
    const a = await Gltf3D.load('assets/models/nature/stump_old.glb', view);
    const b = await Gltf3D.load('assets/models/nature/stump_old.glb', view);
...
test('an unloaded container is not reused: the next load fetches a fresh asset', async () => {
    const { Gltf3D, registry, view } = harness();
    const url = 'assets/models/nature/tree_blocks_fall.glb';
    const first = await Gltf3D.load(url, view);
    assert.equal(registry.added.length, 1);
    // The last instance of the shared container died: the registry unloads the asset and
    // destroys its meshes — exactly what the set transition used to cache behind a promise.
...
test('a failed load is not cached: the retry hits the registry again', async () => {
    const { Gltf3D, registry, view } = harness();
    const url = 'assets/models/actors/missing.glb';
